...
//pthreads version of sequential_triangle_counting(), workload is split between the workers and each worker processes its share of columns
void *worker(void *arg){
     int i,j,work_size,pth_id,idx_start,idx_end;
     pth_id = *(int*)arg; //get the id of the thread
     work_size = M->size/num_threads; //calculate work_size of each thread
     //find column index where each thread should start and finish working
     idx_start = pth_id*work_size; 
     idx_end = pth_id == num_threads-1 ? M->size:(pth_id+1)*work_size; //if its the last thread make sure to include all the columns until the end
     for(i=idx_start;i<idx_end;i++){
          for (j=M->cols_ptrs[i];j<M->cols_ptrs[i+1];j++){
...
int main(int argc, char *argv[])
{
     ...
     int i;
     pthread_t *threads;
     //we expect as arguments the name of the file and the number of threads
     if (argc < 3){
          exit(1);
     }
     else{
     ...
     COOtoCSC(A);
     //create as many threads as we specified
     threads = (pthread_t*)malloc(num_threads*sizeof(pthread_t));
     gettimeofday(&t_start,NULL);
     for(i=0;i<num_threads;i++){
          //send the threads at worker() function and pass their id as argument (so their workload can be calculated)
          int *pth_id;
          pth_id = malloc(sizeof(int));
          *pth_id=i;
          pthread_create(&threads[i],NULL,worker,(void*)pth_id);
     }
     for(i=0;i<num_threads;i++){
