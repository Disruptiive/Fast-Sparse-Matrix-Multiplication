...
//same function as sequential_triangle_counting() in julia
void sequential_triangle_counting(){
     int i,j,row,col,col_start,col_end;
     for(i=0;i<M->size;i++){
          col = i;
          col_start = M->cols_ptrs[i];
          col_end = M->cols_ptrs[i+1];
          for (j=col_start;j<col_end;j++){
               row = M->rows[j];
               M->values[j] = vectorsmultiply(row,col);
          }
     }
}
...
int main(int argc, char *argv[])
{
     ...
     printf("Triangles: %d Execution time; %.3lf s\n",final,exec_time/1000);
     free(M);
     free(final_arr);
