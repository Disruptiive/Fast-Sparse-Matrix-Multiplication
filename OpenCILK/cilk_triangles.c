...
//parallelized version of sequential_triangle_counting(), cilk_for is used in outer and inner loop of the nested loop
void cilk_triangle_counting(){
     int i,j;
     cilk_for(i=0;i<M->size;i++){
          cilk_for(j=M->cols_ptrs[i];j<M->cols_ptrs[i+1];j++){
